...
connection = sqlite3.connect("Tablee.db") 
  
# cursor  
crsr = connection.cursor() 
  
# SQL command to create a table in the database 
sql_command = """CREATE TABLE eesmp (  
...
sql_command = """INSERT INTO eesmp VALUES (21, "Arpit", "Bansal", "M", "2014-03-28");"""
crsr.execute(sql_command) 
  
# another SQL command to insert the data in the table 
sql_command = """INSERT INTO eesmp VALUES (1, "Bill", "Gates", "M", "1980-10-28");"""
crsr.execute(sql_command) 
...
def Submit():
    ...
    crsr = connection.cursor() 
    # Insert Into Table
    crsr.execute("INSERT INTO eesmp VALUES (:Staffno,  :f_name , :l_name , :Gender , :DOJi)",
        {
            'Staffno' : Staffno.get(),
            'f_name' : f_name.get(),
            'l_name' : l_name.get(),
    ...
    messagebox.askokcancel("Do you want to save changes?", "Are you sure you want to save changes?")
...
def Query():
    ...
    crsr = connection.cursor() 
    # Insert Into Table
    crsr.execute("SELECT *,oid FROM eesmp")  
    ...
    records =""
    for record in ans:
        records += record[1] +" " +record[2] +"   " + str(record[0]) + record[3] + "" + record[4] + " " + '\n'
    query_window = Tk()
    Label(query_window , text = records ,bg = "yellow").grid(row = 10 ,column = 0 , columnspan = 2 )
def delete():
    # connecting to the database  
    connection = sqlite3.connect("Tablee.db") 
    # cursor  
    ...
    connection.execute("DELETE from eesmp where oid=" + deli.get())
    deli.delete(0 , END) 
    connection.commit() 
    connection.close()
def edit():
...
def update():
    ...
    crsr = connection.cursor() 
    record_id = deli.get()
    crsr.execute("SELECT * FROM eesmp WHERE oid = " + record_id)
